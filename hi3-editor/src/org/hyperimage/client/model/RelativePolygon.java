...
public class RelativePolygon {
	public static enum HiPolygonTypes {HI_FREEDESIGN, HI_RECTANGLE, HI_CIRCLE, HI_ARROW, HI_FREEHAND, HI_USERDESIGN};
	...
	public RelativePolygon(HiPolygonTypes type, int x, int y, int width, int height, String modelDesign, int scaleX, int scaleY) {
		...
		if ( type == HiPolygonTypes.HI_FREEDESIGN ) {
                    ...
                    addPoint(x - width/2, y - 7);
                    addPoint(x          , y - 7);
                    addPoint(x          , y -14);
                    addPoint(x + width/3, y    );
                    addPoint(x          , y +14);
                    addPoint(x          , y + 7);
                    addPoint(x - width/2, y + 7);
                    addPoint(x - width/2, y - 7);
                    ...
                    int phi=0;
                    while (phi<=640) {
                        xn = width * Math.cos(phi/100.0);
                        yn = width * Math.sin(phi/100.0);
                        int tx = x + (int)xn;
                        int ty = y + (int)yn;
                        addPoint(tx,ty);
                        phi += 6;
	...
	public void setScale(int scaleX, int scaleY) {
        ...
        public void scale(int scaleX, int scaleY) {
                ...
                for (int i = 0; i < pointsX.size(); i++) {
                    pointsX.set(i, (double) pointsX.get(i) * scaleX);
                    pointsY.set(i, (double) pointsY.get(i) * scaleY);
                }
	...
	public void transformBy(double relX, double relY, PolygonResizeCorners corner) {
		...
		if ( pointsX.size() > 0 ) {
			transformPath.moveTo(pointsX.get(0), pointsY.get(0));
			for ( int i=1; i < pointsX.size(); i++)
				transformPath.lineTo(pointsX.get(i), pointsY.get(i));			
		}
	...
	public void transformByAngle(double theta, PolygonResizeCorners corner) {
            ...
            double yk = (double) scaleX / scaleY;
            
       
            GeneralPath transformPath = new GeneralPath();
            if ( pointsX.size() > 0 ) {
                    transformPath.moveTo(pointsX.get(0), pointsY.get(0)*(1/yk));
                    for ( int i=1; i < pointsX.size(); i++)
                            transformPath.lineTo(pointsX.get(i), pointsY.get(i)*(1/yk));
            }
            double oldX = transformPath.getBounds2D().getX();
            double oldY = transformPath.getBounds2D().getY();
	...
	public Point getPoint(int index) {
        ...
        public void setPoints(Point[] points) {
            ...
            for (Point point : points) {
                if ( point != null ) {
                    pointsX.add((double) point.x/scaleX);
                    pointsY.add((double) point.y/scaleY);
                }
            }
            hasChanges = true;
            createDisplayablePath();
	...
	private HiPolygonTypes getTypeFromModel(String modelType) {
		...
		else if ( modelType.startsWith("C") )
			type = HiPolygonTypes.HI_CIRCLE;
		else if ( modelType.startsWith("A") )
			type = HiPolygonTypes.HI_ARROW;
		else
